...
typedef struct {
    struct particle data[MAX_PARTICLES];
    size_t size;
} array_particle_t;
...
#ifdef PLATFORM_PC
void particle_update() {
    for (size_t i = 0; i < particles.size; /* i wird unten angepasst */) {
        ...
        if (blocks[in_block.type]) {
            ...
            size_t count = blocks[in_block.type]->getBoundingBox(&blk, true, NULL);
            if (count > 0) {
                struct AABB aabb[count];
                blocks[in_block.type]->getBoundingBox(&blk, true, aabb);
        ...
        if (p->age > 0) {
            i++; // nur weitermachen, wenn das Partikel nicht gelöscht wird
        } else {
            // Element entfernen: einfach nachfolgende Elemente nach links schieben
            for (size_t j = i; j + 1 < particles.size; j++) {
                particles.data[j] = particles.data[j + 1];
            }
            particles.size--; // Größe des Arrays verringern
            // i bleibt gleich, damit das nächste Partikel jetzt an Index i überprüft wird
        }
    }
}
#endif
