...
static void screen_furnace_update(struct screen* s, float dt) {
	...
	if(input_pressed(IB_INVENTORY, player)) {
		svin_rpc_send(&(struct server_rpc) {
			...
			.payload.window_close.window = furnace_container[player],
		});
	...
	if(input_pressed(IB_GUI_CLICK, player)) {
		uint16_t action_id;
		if(windowc_new_action(gstate_windows()[furnace_container[player]],
		                      &action_id, false, slots[selected_slot[player]].slot)) {
			svin_rpc_send(&(struct server_rpc) {
				RPC_PLAYER_ID(player)
				.type = SRPC_WINDOW_CLICK,
				.payload.window_click.window = furnace_container[player],
...
static void screen_furnace_render2D(struct screen* s, int width, int height) {
	...
	struct inventory* inv
		= windowc_get_latest(gstate_windows()[furnace_container[player]]);
