...
#include <my_text_renderer.h> //text zeichnen
...
uint8_t* tex_read(const char* filename, size_t* width, size_t* height) {
	...
	uint8_t* img;
	unsigned w, h;
	if(lodepng_decode32_file_c(&img, &w, &h, string_get_cstr(tmp))) {
		string_clear(tmp);
		
				video_init_custom();
        	VIDEO_ClearFrameBuffer(rmode, framebuffer, COLOR_WHITE);
        	
        	render_text("tex_read erfolgreich", 30, 30, "green");
		...
		video_init_custom();
        	VIDEO_ClearFrameBuffer(rmode, framebuffer, COLOR_WHITE);
        	render_text("tex_read fehlgeschlagen", 10, 10, "red");
...
void tex_gfx_load_file(struct tex_gfx* tex, const char* filename,
	...
	if(!img) {
		render_text("img = false", 10, 70, "black");
		return;
	} else { render_text("img = true", 10, 90, "black"); }
