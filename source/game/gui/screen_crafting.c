...
static void screen_crafting_update(struct screen* s, float dt) {
	...
	if(input_pressed(IB_INVENTORY, player)) {
		svin_rpc_send(&(struct server_rpc) {
			...
			.payload.window_close.window = crafting_container[player],
		});
	...
	if(input_pressed(IB_GUI_CLICK, player)) {
		uint16_t action_id;
		if(windowc_new_action(gstate_windows()[crafting_container[player]],
		                      &action_id, false, slots[selected_slot[player]].slot)) {
			svin_rpc_send(&(struct server_rpc) {
				RPC_PLAYER_ID(player)
				.type = SRPC_WINDOW_CLICK,
				.payload.window_click.window = crafting_container[player],
...
static void screen_crafting_render2D(struct screen* s, int width, int height) {
	...
	struct inventory* inv
		= windowc_get_latest(gstate_windows()[crafting_container[player]]);
	...
	gutil_texquad(off_x, off_y, 0, 0, GUI_WIDTH, GUI_HEIGHT, GUI_WIDTH * scale,
				  GUI_HEIGHT * scale);
	gutil_text(off_x + 28 * scale, off_y + 6 * scale, "\2478Crafting", 8 * scale, false);
