...
void recipe_init() {
	...
	recipe_add(
		recipes_crafting,
		(struct item_data) {.id = BLOCK_PISTON, .durability = 0, .count = 1},
		3, 3, (uint8_t[]) {1, 1, 1, 2, 3, 2, 2, 4, 2},
		(struct item_data) {.id = BLOCK_PLANKS}, false,
		(struct item_data) {.id = BLOCK_COBBLESTONE}, false,
		(struct item_data) {.id = ITEM_IRON}, false,
		(struct item_data) {.id = ITEM_REDSTONE}, false);
	...
	recipe_add(recipes_crafting,
			   ...
			   (struct item_data) {.id = BLOCK_PLANKS}, false);
	recipe_add(recipes_crafting,
			   (struct item_data) {.id = BLOCK_STONE_BUTTON,
								   .durability = 0,
								   .count = 1},
			   1, 1, (uint8_t[]) {1}, (struct item_data) {.id = BLOCK_STONE},
			   false);
	recipe_add(
		recipes_crafting,
		(struct item_data) {.id = BLOCK_GLASS_PANE, .durability = 0, .count = 16},
		3, 2, (uint8_t[]) {1, 1, 1, 1, 1, 1},
		(struct item_data) {.id = BLOCK_GLASS}, false);
	recipe_add(
		recipes_crafting,
		(struct item_data) {.id = BLOCK_MELON, .durability = 0, .count = 1}, 3, 3,
		(uint8_t[]) {1, 1, 1, 1, 1, 1, 1, 1, 1},
		(struct item_data) {.id = ITEM_MELON}, false);
	recipe_add(recipes_crafting,
			   (struct item_data) {.id = BLOCK_REPEATER_OFF,
								   .durability = 0,
								   .count = 1},
			   3, 2, (uint8_t[]) {1, 2, 1, 3, 3, 3},
			   (struct item_data) {.id = BLOCK_REDSTONE_TORCH_LIT}, false,
			   (struct item_data) {.id = ITEM_REDSTONE}, false,
			   (struct item_data) {.id = BLOCK_STONE}, false);
	recipe_add(recipes_crafting,
			   (struct item_data) {.id = BLOCK_STONE_BRICKS,
								   .durability = 0,
								   .count = 4},
			   2, 2, (uint8_t[]) {1, 1, 1, 1},
			   (struct item_data) {.id = BLOCK_STONE}, false);
	recipe_add(recipes_crafting,
			   (struct item_data) {.id = BLOCK_IRON_BARS,
								   .durability = 0,
								   .count = 16},
			   3, 2, (uint8_t[]) {1, 1, 1, 1, 1, 1},
			   (struct item_data) {.id = ITEM_IRON}, false);
	...
	recipe_add(recipes_crafting,
			   (struct item_data) {.id = BLOCK_CAULDRON,
								   .durability = 0,
								   .count = 1},
			   3, 3, (uint8_t[]) {1, 0, 1, 1, 0, 1, 1, 1, 1},
			   (struct item_data) {.id = ITEM_IRON}, false);
	recipe_add(
		recipes_crafting,
